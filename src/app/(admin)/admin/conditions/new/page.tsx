    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-[1000px] mx-auto pb-24">
      <div className="flex items-center justify-between">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease }} className="flex items-center gap-4">
          <Link href="/admin/conditions" className="p-2 bg-paper border border-line rounded-xl hover:bg-porcelain transition-colors text-ink">
            <ArrowLeft size={16} />
          </Link>
          <div>
            <p className="text-petrol-300 text-sm">Health Conditions</p>
            <h2 className="font-display font-bold text-2xl text-ink">Create Condition</h2>
          </div>
        </motion.div>
        ...
        >
          {isSubmitting ? 'Saving...' : <><Save size={16} /> Save Condition</>}
        </button>
      </div>
      ...
      <div className="grid lg:grid-cols-[2fr_1fr] gap-6 items-start">
        <div className="space-y-6">
          <div className="bg-paper rounded-2xl border border-line shadow-sm p-6 space-y-5">
            ...
            <div>
              ...
              <input required name="name" value={formData.name} onChange={handleChange} className="w-full px-4 py-2.5 bg-white border border-line rounded-xl focus:outline-none focus:border-petrol focus:ring-1 focus:ring-petrol transition-shadow" placeholder="e.g. Hypertension" />
            </div>
            <div>
              <label className="block text-sm font-medium text-ink mb-1.5">Description</label>
              <textarea name="description" value={formData.description} onChange={handleChange} rows={5} className="w-full px-4 py-2.5 bg-white border border-line rounded-xl focus:outline-none focus:border-petrol focus:ring-1 focus:ring-petrol transition-shadow" placeholder="Detailed medical condition description..." />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-paper rounded-2xl border border-line shadow-sm p-6 space-y-5">
            <h3 className="font-semibold text-ink text-lg">Featured Image</h3>
            <div 
